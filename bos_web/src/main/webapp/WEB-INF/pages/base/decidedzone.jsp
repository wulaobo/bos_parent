...
<script type="text/javascript">
	...
	function doAssociations(){
        ...
        //alert(v1.length);
		if(array.length != 1){
		    $.messager.alert("提示信息","请选择一条信息","warning");
		}else{
           $('#customerWindow').window('open');
            //清除下拉列表的元素
            $("#noassociationSelect").empty();
		    $.post("${ pageContext.request.contextPath }/DecidedZoneAction_findAllNotAssociation.action",function (data) {
				//alert(data);
			...
			var id = row.id;
			//alert(id);
            $.post("${ pageContext.request.contextPath }/DecidedZoneAction_findAllHasAssociation.action",{id:id},function (data) {
                //alert(data);
                for(var i = 0;i<data.length;i++) {
                    var id = data[i].id;
                    var name = data[i].name;
	...
	$(function(){
		...
		$('#grid').datagrid({
			...
			pagination : true,
			toolbar : toolbar,
			url : "${ pageContext.request.contextPath }/DecidedZoneAction_QueryPage.action",
			idField : 'id',
			columns : columns,
	...
	function doDblClickRow(index,data){
		...
		$('#association_subarea').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "${ pageContext.request.contextPath }/SubareaAction_findListSubareaByDecidedzone_id?decidedzone_id="+data.id,
			columns : [ [{
				field : 'id',
				title : '分拣编号',
				width : 120,
		...
		$('#association_customer').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "${ pageContext.request.contextPath }/DecidedZoneAction_findAllHasAssociation.action?id="+data.id,
			columns : [[{
				field : 'id',
				title : '客户编号',
				width : 120,
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="定区添加修改" id="addDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		<div style="height:31px;overflow:hidden;" split="false" border="false" >
			<div class="datagrid-toolbar">
				...
				<script type="text/javascript">
					 $(function () {
						 $("#save").click(function () {
				             var v = $("#StaffAndSubareaAssociatedForm").form("validate");
				             if(v){
                                 $("#StaffAndSubareaAssociatedForm").submit();
							 }
                         });
                     });
				</script>
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="StaffAndSubareaAssociatedForm" method="post" action="DecidedZoneAction_add">
					...
					<tr>
						...
						<td>
							<input class="easyui-combobox" name="staff.id"
    							data-options="valueField:'id',textField:'name',url:'${ pageContext.request.contextPath }/StaffAction_findAllNotDeleteStaff.action'" />
						</td>
					</tr>
					<tr height="300">
						...
						<td>
							<table id="subareaGrid"  class="easyui-datagrid" border="false" style="width:300px;height:300px"
								   data-options="url:'${pageContext.request.contextPath}/SubareaAction_findAllSubareaNotAssociated',fitColumns:true,singleSelect:false">
								<thead>
							        <tr>
							...
							<script type="text/javascript">
                                $(function () {
                                    ...
                                    $("#associationBtn").click(function () {
                                        //获取数据表格中通过点击选中的行数据 (也就是选中的定区)
                                        var row = $("#grid").datagrid("getSelected");
                                        //把选中定区的id赋值给隐藏域中的id
                                        $("#customerDecidedZoneId").val(row.id);
                                        //alert($("#noassociationSelect").val());
                                        //选中右侧下拉列表中的选项
